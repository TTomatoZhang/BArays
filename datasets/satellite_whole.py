...
def get_rays(cols, rows, rpc, min_alt, max_alt):
    ...
    x_near, y_near = sat_utils.utm_from_latlon(lats_high, lons_high)
    ...
    x_far, y_far = sat_utils.utm_from_latlon(lats_low, lons_low)
...
class SatelliteDataset(Dataset):
    def __init__(self, root_dir, split="train", dname='JAX_179',  img_downscale=1.0, cache_dir='/home/aipt/Documents/ztt/satdata/satmvsdata/rays'):
        """
        ...
        self.json_dir = os.path.join(root_dir, 'cams_ba')  # root_dir
        self.img_dir = os.path.join(root_dir, 'rgb')  # img_dir
        self.cache_dir = os.path.join(cache_dir, dname + '_whole')
        ...
        # load scaling params
        if not os.path.exists(f"{self.cache_dir}/scene.loc"):
            self.init_scaling_params()
        d = sat_utils.read_dict_from_json(os.path.join(self.cache_dir, "scene.loc"))
    ...
    def load_train_split(self):
        ...
        self.all_rays, self.all_rgbs, self.all_ids, self.all_rpcs = self.load_data(self.json_files, verbose=True)
    ...
    def init_scaling_params(self):
        ...
        sat_utils.write_dict_to_json(d, f"{self.cache_dir}/scene.loc")
        print("... done !")
    ...
    def load_data(self, json_files, verbose=False):
        ...
        self.rids = []
        rshapes = 0
        all_rgbs, all_rays, all_sun_dirs, all_ids, all_rpcs = [], [], [], [], []
        for t, json_p in enumerate(json_files):
            ...
            try:
                ...
                sun_dirs = None
            all_rpcs += [sat_utils.get_tensor_from_rpc(rpc)]
            all_ids += [t * torch.ones(rays.shape[0], 1)]
            all_rgbs += [rgbs]
            all_rays += [rays]
            rshapes += rays.shape[0]
            self.rids.append(rshapes)
            if sun_dirs != None:
                all_sun_dirs += [sun_dirs]
            if verbose:
                print("Image {} loaded ( {} / {} )".format(img_id, t + 1, len(json_files)))

        latoff_last = rpc.lat_offset
        lonoff_last = rpc.lon_offset
        # get the zone number
        self.zone = utm.latlon_to_zone_number(latoff_last, lonoff_last)

        all_ids = torch.cat(all_ids, 0).type(torch.long)
        all_rays = torch.cat(all_rays, 0)  # (len(json_files)*h*w, 8)
        all_rgbs = torch.cat(all_rgbs, 0)  # (len(json_files)*h*w, 3)
        all_rpcs = torch.cat(all_rpcs, 0)
        if sun_dirs != None:
        ...
        return all_rays, all_rgbs, all_ids, all_rpcs
    ...
    def get_latlonalt_from_nerf_prediction(self, rays, depth):
        ...
        # denormalize prediction to obtain utm coordinates
        ...
        lats, lons = sat_utils.latlon_from_utm(xyz[:, 0], xyz[:, 1], zone_number=self.zone, hemisphere='N')
        alts = xyz[:, 2]
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            # imid =
            # sample = {"rays": self.all_rays[idx], "rpc":self.all_rpcs[self.all_ids[idx]], "rgbs": self.all_rgbs[idx], }  # "ts": self.all_ids[idx].long()
            sample = {"rays": self.all_rays[idx],
                      "rgbs": self.all_rgbs[idx],
                      "ts": self.all_ids[idx]  # .long()
                      }
        else:
            rays, rgbs, _, _ = self.load_data([self.json_files[idx]])
            # ts = self.all_ids[idx] * torch.ones(rays.shape[0], 1)
            d = sat_utils.read_dict_from_json(self.json_files[idx])
            img_id = sat_utils.get_file_id(d["img"])
            h, w = int(d["height"] // self.img_downscale), int(d["width"] // self.img_downscale)
            sample = {"rays": rays, "rgbs": rgbs, # "ts": ts.long(),
                      "src_id": img_id, "h": h, "w": w}
        return sample
