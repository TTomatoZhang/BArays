...
class MaskedAutoencoderGroupChannelViT(nn.Module):
    ...
    def __init__(self, img_size=224, 
                 patch_size=16, in_chans=3, spatial_mask=False,
...
def mae_vit_base_patch16_gc(**kwargs):
...
def mae_vit_large_patch16_gc(**kwargs):
    model = MaskedAutoencoderGroupChannelViT(
        channel_embed=256, embed_dim=1024, depth=24, num_heads=16,
        decoder_channel_embed=128, decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
mae_vit_base_patch16 = mae_vit_base_patch16_gc  # decoder: 512 dim, 8 blocks
mae_vit_large_patch16 = mae_vit_large_patch16_gc  # decoder: 512 dim, 8 blocks
